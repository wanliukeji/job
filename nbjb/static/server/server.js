...
for (let i = 0; i < sql.length; i++) {
  ...
  for (let j = 0; j < item.subLevelModelList.length; j++) {
    ...
    for (let k = 0; k < sub.subLevelModelList.length; k++) {
      ...
      let addSql = 'INSERT INTO job(code,name,firstChar,pinyin) VALUES(?,?,?,?)';
      var param = [son.code, son.name, son.firstChar, son.pinyin];
    ...
    let addSql = 'INSERT INTO job(code,name,firstChar,pinyin) VALUES(?,?,?,?)';
    var param = [sub.code, sub.name, sub.firstChar, sub.pinyin];
    connection.query(addSql, param, function (err, rs) {
      if (err) {
        console.log(err.message);
        return;
  ...
  let addSql = 'INSERT INTO job(code,name,firstChar,pinyin) VALUES(?,?,?,?)';
  var param = [item.code, item.name, item.firstChar, item.pinyin];
  connection.query(addSql, param, function (err, rs) {
    if (err) {
      console.log(err.message);
      return;
...
// connection.query('select * from job', function (err, rs) {
//   if (err) {
//     console.log(err);
//     return;
//   }
